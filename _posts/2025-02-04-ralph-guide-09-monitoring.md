---
layout: post
title: "Ralph 가이드 09 - 모니터링"
date: 2025-02-04
categories: [AI, Claude Code, Ralph]
tags: [ralph, monitoring, tmux, dashboard, logs]
permalink: /ralph-guide-09-monitoring/
---

# 모니터링

## tmux 통합

### 모니터링 모드 시작

```bash
ralph --monitor
```

이 명령은 tmux 세션을 생성하고 두 개의 패널을 표시합니다:

```
┌─────────────────────────────────────────────────────────────┐
│                     RALPH MONITOR                            │
├──────────────────────────────┬──────────────────────────────┤
│                              │                               │
│  Ralph Loop Output           │  Monitoring Dashboard         │
│  (왼쪽 패널)                   │  (오른쪽 패널)                 │
│                              │                               │
│  Claude Code 실행 로그         │  세션 상태                    │
│  작업 진행 상황                │  진행률                       │
│  테스트 결과                   │  API 사용량                   │
│  에러 메시지                   │  서킷 브레이커 상태             │
│                              │                               │
└──────────────────────────────┴──────────────────────────────┘
```

### 패널 구조

```
┌────────────────────────────────┬───────────────────────────────┐
│ Left Pane (60%)                │ Right Pane (40%)              │
├────────────────────────────────┼───────────────────────────────┤
│                                │  ╔═══════════════════════════╗│
│ [10:30:15] Ralph loop started  │  ║ SESSION DASHBOARD        ║│
│ [10:30:16] Reading PROMPT.md   │  ╠═══════════════════════════╣│
│ [10:30:16] Reading fix_plan.md │  ║ Status:  RUNNING         ║│
│ [10:30:17] Found 5 unchecked   │  ║ Session: proj-abc123     ║│
│            tasks               │  ║ Loop:    15 / 200        ║│
│ [10:30:18] Starting task:      │  ║ Time:    1h 23m          ║│
│            "Create API route"  │  ╠═══════════════════════════╣│
│                                │  ║ PROGRESS                 ║│
│ > claude code running...       │  ║ Tasks: ████████░░ 8/12   ║│
│                                │  ║ Tests: ✓ Passing         ║│
│ [10:32:45] Task completed      │  ╠═══════════════════════════╣│
│ [10:32:46] Running tests...    │  ║ API USAGE                ║│
│                                │  ║ Hour:  ███████░░ 67/100  ║│
│ > npm test                     │  ║ Total: 234 calls         ║│
│   PASS  src/api.test.js        │  ╠═══════════════════════════╣│
│   ✓ GET /users (24ms)          │  ║ CIRCUIT BREAKER          ║│
│   ✓ POST /users (18ms)         │  ║ State:  ● CLOSED         ║│
│                                │  ║ Errors: 0 / 5            ║│
│ [10:33:02] All tests passed    │  ╠═══════════════════════════╣│
│ [10:33:03] Updating fix_plan   │  ║ LAST ACTIVITY            ║│
│ [10:33:04] Starting next task  │  ║ 10:33:04 - Starting      ║│
│                                │  ║           next task      ║│
└────────────────────────────────┴───────────────────────────────┘
```

### tmux 키 바인딩

| 키 조합 | 동작 |
|---------|------|
| `Ctrl+B, D` | 세션 분리 (Ralph 계속 실행) |
| `Ctrl+B, [` | 스크롤 모드 (q로 종료) |
| `Ctrl+B, 0` | 왼쪽 패널로 이동 |
| `Ctrl+B, 1` | 오른쪽 패널로 이동 |
| `Ctrl+B, z` | 현재 패널 확대/축소 |
| `Ctrl+B, %` | 수직 분할 |
| `Ctrl+B, "` | 수평 분할 |

### 세션 관리

```bash
# 실행 중인 tmux 세션 확인
tmux ls

# 세션에 다시 연결
tmux attach -t my-project

# 세션 종료 (Ralph도 종료)
tmux kill-session -t my-project
```

## 라이브 대시보드

### 대시보드 요소

```
╔═══════════════════════════════════════════════════════════════╗
║                      RALPH DASHBOARD                           ║
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  SESSION INFO                                                  ║
║  ├─ ID:        my-project-abc123                              ║
║  ├─ Status:    ● RUNNING                                      ║
║  ├─ Started:   2024-01-15 10:00:00                            ║
║  └─ Duration:  1h 45m 23s                                     ║
║                                                                ║
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  PROGRESS                                                      ║
║  ├─ Loop:      23 / 200                                       ║
║  ├─ Tasks:     ████████████████░░░░ 8/12 (67%)                ║
║  └─ Current:   "Implement user authentication"                 ║
║                                                                ║
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  API USAGE (resets in 15m)                                    ║
║  ├─ This Hour: █████████████░░░░░░░ 67/100 (67%)              ║
║  └─ Total:     234 calls                                       ║
║                                                                ║
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  CIRCUIT BREAKER                                               ║
║  ├─ State:     ● CLOSED (normal)                              ║
║  ├─ Errors:    1/5                                            ║
║  └─ Last Err:  test timeout (32m ago)                         ║
║                                                                ║
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  RECENT ACTIVITY                                               ║
║  ├─ 10:45:30  ✓ Task completed: "Add user model"              ║
║  ├─ 10:45:31  ◎ Tests passed (12 tests)                       ║
║  ├─ 10:45:32  → Starting: "Implement user auth"               ║
║  └─ 10:45:33  📝 Reading specs/auth.md                        ║
║                                                                ║
╚═══════════════════════════════════════════════════════════════╝
```

### 상태 표시기

| 아이콘 | 의미 |
|--------|------|
| `●` | 활성 상태 |
| `○` | 비활성 상태 |
| `✓` | 성공/완료 |
| `✗` | 실패/에러 |
| `→` | 진행 중 |
| `⏸` | 일시 중지 |
| `◎` | 테스트 관련 |
| `📝` | 파일 읽기/쓰기 |

### 색상 코드

| 색상 | 의미 |
|------|------|
| 녹색 | 정상/성공 |
| 노랑 | 경고/주의 |
| 빨강 | 에러/실패 |
| 파랑 | 정보/진행 중 |
| 회색 | 비활성 |

## 로그 시스템

### 로그 파일 구조

```
.ralph/logs/
├── session_abc123.log      # 세션별 전체 로그
├── session_def456.log      # 이전 세션 로그
├── session_latest.log      # 최신 세션 심볼릭 링크
├── error.log               # 에러 전용 로그
└── api_calls.log           # API 호출 기록
```

### 로그 레벨

| 레벨 | 설명 | 예시 |
|------|------|------|
| `DEBUG` | 상세 디버깅 정보 | 변수 값, 함수 호출 |
| `INFO` | 일반 진행 정보 | 작업 시작/완료 |
| `WARN` | 경고 (계속 진행) | 재시도, 느린 응답 |
| `ERROR` | 에러 (복구 시도) | 테스트 실패, API 오류 |
| `FATAL` | 치명적 (중지) | 인증 실패, 설정 오류 |

### 로그 형식

```
[2024-01-15T10:30:15.123Z] [INFO] [loop:15] Task started: "Create API route"
[2024-01-15T10:30:45.456Z] [DEBUG] [loop:15] File written: src/routes/users.js
[2024-01-15T10:31:02.789Z] [INFO] [loop:15] Tests passed: 12/12
[2024-01-15T10:31:03.012Z] [INFO] [loop:15] Task completed: "Create API route"
[2024-01-15T10:31:03.345Z] [WARN] [loop:16] API call slow: 5.2s (threshold: 3s)
```

### 로그 확인 명령

```bash
# 실시간 로그 스트리밍
tail -f .ralph/logs/session_latest.log

# 에러만 필터링
grep "\[ERROR\]" .ralph/logs/session_latest.log

# 특정 시간대 로그
grep "2024-01-15T10:3" .ralph/logs/session_latest.log

# 특정 작업 관련 로그
grep "Create API route" .ralph/logs/session_latest.log

# 최근 100줄
tail -100 .ralph/logs/session_latest.log
```

### 로그 레벨 설정

```bash
# .ralphrc에서 설정
LOG_LEVEL="debug"

# 환경 변수로 설정
RALPH_LOG_LEVEL=debug ralph --monitor

# 명령줄 옵션
ralph --verbose  # debug 레벨 활성화
```

## CLI 상태 확인

### ralph --status

```bash
ralph --status
```

출력:
```
Ralph Session Status
====================
Session ID:  my-project-abc123
Status:      ● RUNNING
Started:     2024-01-15 10:00:00 (1h 45m ago)

Progress
--------
Loop:        23 / 200
Tasks:       8 / 12 completed (67%)
Current:     "Implement user authentication"

API Usage
---------
This Hour:   67 / 100 calls (67%)
Resets In:   15 minutes
Total:       234 calls

Circuit Breaker
---------------
State:       CLOSED
Error Count: 1 / 5
Last Error:  test timeout (32m ago)

Files Modified (last hour)
--------------------------
  src/routes/users.js
  src/models/user.js
  tests/users.test.js
```

### JSON 출력

```bash
ralph --status --json
```

```json
{
  "session": {
    "id": "my-project-abc123",
    "status": "RUNNING",
    "started_at": "2024-01-15T10:00:00Z",
    "duration_seconds": 6323
  },
  "progress": {
    "loop": 23,
    "max_loops": 200,
    "tasks_completed": 8,
    "tasks_total": 12,
    "current_task": "Implement user authentication"
  },
  "api_usage": {
    "calls_this_hour": 67,
    "max_calls_per_hour": 100,
    "resets_in_seconds": 900,
    "calls_total": 234
  },
  "circuit_breaker": {
    "state": "CLOSED",
    "error_count": 1,
    "error_threshold": 5
  }
}
```

## 외부 모니터링 통합

### Webhook 알림

```bash
# .ralphrc
WEBHOOK_URL="https://hooks.slack.com/services/xxx/yyy/zzz"
NOTIFY_ON_COMPLETE=true
NOTIFY_ON_ERROR=true
```

### 커스텀 스크립트

```bash
# .ralph/hooks/on_complete.sh
#!/bin/bash
curl -X POST $WEBHOOK_URL \
  -H 'Content-type: application/json' \
  -d "{\"text\": \"Ralph completed: $PROJECT_NAME\"}"

# .ralph/hooks/on_error.sh
#!/bin/bash
curl -X POST $WEBHOOK_URL \
  -H 'Content-type: application/json' \
  -d "{\"text\": \"Ralph error: $ERROR_MESSAGE\"}"
```

---

**이전 장:** [세션 관리](/ralph-guide-08-session/) | **다음 장:** [베스트 프랙티스](/ralph-guide-10-best-practices/)
